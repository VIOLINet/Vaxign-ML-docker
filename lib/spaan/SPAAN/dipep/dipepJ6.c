...
#define N 10000
#define cut_off 2.0
FILE *fin, *fout, *foutNN, *foutsign, *dipepcluster;
char seq[M];
int STOP;
int adhesin;
...
//			if(local_count[i][j]>0)
//				printf("%c%c=%d %.2f  ",AA[i],AA[j],local_count[i][j],total_count[adhesin][i][j]);
		}
...
int main(int argc, char *argv[])
{
	...
	foutsign=fopen("significant_dipep","w");
	dipepcluster=fopen("dipepcluster1.txt","w");
	...
	for(i=0;i<20;i++)
		for(j=0;j<20;j++)
		{
			...
			{
				fprintf(dipepcluster,"%f\n",(total_count[0][i][j]-total_count[1][i][j])/total_count[1][i][j]);
				if( ((total_count[0][i][j]-total_count[1][i][j])/total_count[1][i][j]) >= cut_off)
				{
					significance[i][j]=1;
					significance_count++;
				}
			...
			{
				fprintf(dipepcluster,"%f\n",(total_count[1][i][j]-total_count[0][i][j])/total_count[0][i][j]);
				if((total_count[1][i][j] > total_count[0][i][j]))
					if( ((total_count[1][i][j]-total_count[0][i][j])/total_count[0][i][j]) >= cut_off)
					{
						significance[i][j]=1;
						significance_count++;
					}
	...
	for(i=0;i<20;i++)
		for(j=0;j<20;j++)
			fprintf(foutsign,"%d ",significance[i][j]);
